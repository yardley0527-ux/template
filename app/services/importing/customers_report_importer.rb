  ...
  class CustomersReportImporter
    ...
    def call
      ...
      ((@header_row + 1)..last).each do |row_i|
        ...
        shopline_id = payload[:shopline_id].to_s.strip.presence
        ...
        # source_row_hash: 只當「來源追溯」，不要用它來 find_or_initialize
        ...
        customer =
          if shopline_id
            ShoplineCustomer.find_or_initialize_by(shopline_id: shopline_id)
          else
            ShoplineCustomer.find_or_initialize_by(email: email)
          end

        customer.assign_attributes(payload.compact)
        customer.import_run_id = run.id if customer.respond_to?(:import_run_id=)
        customer.source_row_hash = row_hash if customer.respond_to?(:source_row_hash=)
        customer.raw = raw.compact if customer.respond_to?(:raw=)
        customer.save!
    ...
    def normalize_phone_cell(v)
      ...
      # Roo 會把純數字欄位讀成 Float：983018856.0
      if v.is_a?(Float)
        v = v.to_i.to_s
      else
        v = v.to_s
