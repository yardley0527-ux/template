...
class WeeklyBriefingService
  ...
  def call
    ...
    previous_week_flags = WeeklyBriefing.find_by(week_start: @period.prev_week_start)&.risk_flags || []
    risk_flags = WeeklyRiskFlagDetector.call(metrics, previous_week_flags: previous_week_flags)
    ...
    base_meta = {
      "risk_flags" => risk_flags, "status_classification" => status,
    ...
    if api_key.blank?
      briefing.update!(status: "failed", metrics: metrics,
                        meta: base_meta.merge("ai_api_success" => false),
                        error_message: "ANTHROPIC_API_KEY 未設定")
      return briefing
    end

    prompt = build_prompt(metrics, risk_flags, status, business_signals, headline)
    ...
    unless validation[:valid]
      briefing.update!(
        ...
        meta: base_meta.merge(
          "ai_api_success" => true, "missing_fields" => validation[:missing_fields], "retried" => retried,
    ...
    # 「一、最大風險不能只依賴Prompt」：不是只改prompt文字碰運氣，AI回應
    # 通過結構驗證後，程式再獨立比對它有沒有真的點名最高優先級的紅燈
    # 主題——沒有就先重試一次（帶著明確缺什麼的提示），仍然沒有就直接
    # 覆寫biggest_risk，不讓漏掉重點的版本上線。
    mandatory_topics = WeeklyMandatoryRiskTopics.call(risk_flags)
    coverage = WeeklyMandatoryRiskCoverageChecker.call(ai_report: ai_report, mandatory_topics: mandatory_topics)
    mandatory_risk_retried = false

    if mandatory_topics.any? && !coverage["covered"]
      mandatory_risk_retried = true
      retry_prompt2 = build_mandatory_risk_retry_prompt(prompt, coverage["uncovered_topics"])
      raw2 = call_claude(retry_prompt2, api_key)
      parsed2 = parse_response(raw2)
      validation2 = WeeklyBriefingResponseValidator.call(parsed2)

      if validation2[:valid]
        parsed2["executive_summary"] = status.merge(parsed2["executive_summary"] || {})
        ai_report2 = parsed2.except("todos")
        coverage2 = WeeklyMandatoryRiskCoverageChecker.call(ai_report: ai_report2, mandatory_topics: mandatory_topics)
        if coverage2["covered"]
          parsed = parsed2
          ai_report = ai_report2
          coverage = coverage2
        else
          coverage = coverage2 # 更新uncovered_topics用重試後的結果，即使還是沒過
        end
      end
    end
    ...
    if mandatory_topics.any? && !coverage["covered"]
      fallback = WeeklyMandatoryRiskFallbackBuilder.call(uncovered_topics: coverage["uncovered_topics"])
      if fallback
        ai_report["executive_summary"]["biggest_risk"] = fallback
        fallback_applied = true
      end
    end

    mandatory_risk_coverage = coverage.merge(
      "retried" => mandatory_risk_retried, "fallback_applied" => fallback_applied
    )

    # AI實際輸入context（跟prompt裡塞給Claude的JSON區塊同一份資料）自動掃描
    ...
    ActiveRecord::Base.transaction do
      briefing.update!(
        ...
        meta:           base_meta.merge("quality_check" => quality_check, "ai_api_success" => true, "retried" => retried)
      )
    ...
    briefing.update!(status: "failed", metrics: metrics || briefing.metrics.presence || {},
                      meta: defined?(base_meta) && base_meta ? base_meta.merge("ai_api_success" => false) : briefing.meta,
  ...
  def build_mandatory_risk_retry_prompt(original_prompt, uncovered_topics)
    labels = uncovered_topics.map { |t| t["label"] }.join("、")
    original_prompt + <<~RETRY

      ＝＝ 重試提示（風險涵蓋）＝＝
      你上一次的回應完全沒有在「biggest_risk」「decisions」「action_items」任何一處提到以下
      已由程式判定為本週最高優先級的風險主題：
      #{labels}
      請重新輸出完整、符合原本格式的JSON，這次務必在 biggest_risk 或至少一項 decision 裡明確點名上述主題
      （如果有兩個以上主題，可以組成一句複合描述，例如「A與B同時發生」，不需要分開寫成兩句）。
    RETRY
  end
  ...
  # 讀 ENV 而不是凍結成class-load-time常數，才能真的做到「可用設定切換」
  # （改ENV就生效，不用重新載入class）；平常呼叫路徑上只讀一次env var，
  # 成本可忽略。給非支援版本一律 fallback 回v6，不讓打錯字的ENV值悄悄跑到
  # 一個不存在的prompt分支。
  def prompt_version
    v = ENV["WEEKLY_BRIEFING_PROMPT_VERSION"]
    SUPPORTED_PROMPT_VERSIONS.include?(v) ? v : DEFAULT_PROMPT_VERSION
  end
  ...
  def critical_data_gaps(metrics)
    Array(metrics.dig("data_gaps", "gaps")).select { |g| g["impact"] == "critical" }
  end

  # v5：2026-09-15第四輪修正的版本，凍結保留供緊急回退用（見 PROMPT_VERSION
  # 常數註解）。跟v6的差異只有：(1) 沒有四大經營燈號／週型標題兩段context、
  # (2) 規則只到17條，沒有v6新增的18~20條。往後v5只做「回退」用途，不再
  # 跟著v6一起改動；要調整既有規則請改v6，除非確認v5本身有bug需要修。
  def build_prompt_v5(metrics, risk_flags, status)
    labeled_flags = labeled_risk_flags(risk_flags)
    critical_gaps = critical_data_gaps(metrics)
  ...
  def build_prompt_v6(metrics, risk_flags, status, business_signals, headline)
    labeled_flags = labeled_risk_flags(risk_flags)
    critical_gaps = critical_data_gaps(metrics)
    ...
    <<~PROMPT
      ...
      ＝＝ 四大經營燈號（程式已經算好顏色，不是你判斷；你的敘述不能跟這裡的顏色矛盾）＝＝
      #{business_signals.to_json}

      ＝＝ 本週週型標題（程式已經算好，直接引用，不要自己改用別的說法）＝＝
      #{headline.to_json}
      ...
      16. 同一個資料缺口全文只能提一次（例如「沒有廣告資料」只在new_and_returning_customers第一次出現時簡短標註，其他段落不要重複）。
      17. 每項decision都要標confidence；low信心不代表不能提決策，改用decision_type="small_test"降低風險，不要因為信心低就不寫。
      18. one_liner必須交代：本週是變好還是變壞、主要影響因素是什麼、是否受活動高基期或資料缺漏影響、最需要處理的問題是什麼——不能只重述數字本身。
      19. 你的任何敘述都不能跟上面「四大經營燈號」的顏色矛盾（例如燈號是紅燈時，敘述不能寫成「表現正常」）；週型標題直接引用上面算好的「本週週型標題」，不要自己重新命名。
      20. revenue_change_breakdown必須明確指出本週下降/成長主要來自購買人數、客單價，或兩者同時（對應上面new_vs_returning.decomposition.decline_driver），不能只寫「營收下降」帶過。
    PROMPT
  end
