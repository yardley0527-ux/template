class ReloveKocsController < ApplicationController
  ...
  def index
    @message_template = ReloveKocMessageTemplate.current
    ...
    @hidden_count = ReloveKoc.hidden_only.count
    @status_counts = ReloveKoc.group(:status).count
    @shipped_by_status = ReloveKoc.pr_gift_shipped.group(:status).count
  ...
  def update_message_template
    ReloveKocMessageTemplate.current.update(content: params[:content])
    redirect_to relove_kocs_path, notice: "已更新發送訊息內容"
  end
  ...
  def toggle_hidden
    ...
    redirect_back fallback_location: relove_kocs_path, allow_other_host: false,
