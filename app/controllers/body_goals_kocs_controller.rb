class BodyGoalsKocsController < ApplicationController
  ...
  def index
    @message_template = BodyGoalsKocMessageTemplate.current
    ...
    @shipped_by_status = BodyGoalsKoc.pr_gift_shipped.group(:status).count
  ...
  def update_message_template
    BodyGoalsKocMessageTemplate.current.update(content: params[:content])
    redirect_to body_goals_kocs_path, notice: "已更新發送訊息內容"
  end
