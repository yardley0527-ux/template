class DianbopopoKocsController < ApplicationController
  PER_PAGE = 10
  ...
  def index
    @message_template = DianbopopoKocMessageTemplate.current

    @sort = params[:sort]
    @kocs = @sort == "likes" ? DianbopopoKoc.order(Arel.sql("COALESCE(max_likes, 0) DESC")) : DianbopopoKoc.ordered_by_engagement
    ...
    @total_count = DianbopopoKoc.count
    @paid_count  = DianbopopoKoc.where(has_paid_partnership: true).count
    @hidden_count = DianbopopoKoc.hidden_only.count
    @status_counts = DianbopopoKoc.group(:status).count
    @shipped_by_status = DianbopopoKoc.pr_gift_shipped.group(:status).count
  ...
  def update_message_template
    DianbopopoKocMessageTemplate.current.update(content: params[:content])
    redirect_to dianbopopo_kocs_path, notice: "已更新發送訊息內容"
  end

  def create
    @koc = DianbopopoKoc.new(koc_params)
    @koc.source = "手動新增"

    if @koc.save
      redirect_to dianbopopo_kocs_path, notice: "已新增 #{@koc.ig_username}"
    else
      redirect_to dianbopopo_kocs_path, alert: @koc.errors.full_messages.join("、")
    end
  end

  def update
    @koc = DianbopopoKoc.find(params[:id])
    @koc.update(koc_params)
    redirect_back fallback_location: dianbopopo_kocs_path, allow_other_host: false, notice: "已更新 #{@koc.ig_username}"
  end
  ...
  def destroy
    ...
    @koc = DianbopopoKoc.find(params[:id])
    @koc.destroy
    redirect_to dianbopopo_kocs_path, notice: "已刪除 #{@koc.ig_username}"
  end
  ...
  def toggle_hidden
    ...
    redirect_back fallback_location: dianbopopo_kocs_path, allow_other_host: false,
  ...
  def koc_params
    ...
    params.require(:dianbopopo_koc).permit(*permitted)
  end
end
