class PodcastContactsController < ApplicationController
  PER_PAGE = 10
  ...
  def create
    @contact = PodcastContact.new(contact_params)
    @contact.source = "手動新增"

    if @contact.save
      redirect_to podcast_contacts_path, notice: "已新增 #{@contact.ig_username}"
    else
      redirect_to podcast_contacts_path, alert: @contact.errors.full_messages.join("、")
    end
  end
  ...
  def destroy
    ...
    @contact = PodcastContact.find(params[:id])
    @contact.destroy
    redirect_to podcast_contacts_path, notice: "已刪除 #{@contact.ig_username}"
  end
  ...
  def contact_params
    ...
    params.require(:podcast_contact).permit(*permitted)
  end
end
